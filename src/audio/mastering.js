...
function audioScoreScheduler(){
  ...
  while(AUDIO_SCORE.next<now+.22&&guard++<5){
    const step=AUDIO_SCORE.step++,offset=cfg.pattern[step%cfg.pattern.length],root=audioHarmonyRoot(),pressure=clamp(combatMusicIntensity||0,0,1),beat=60/(cfg.bpm+(boss?12:Math.round(pressure*10)))/2;
