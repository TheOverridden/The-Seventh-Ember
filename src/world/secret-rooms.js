...
function secretRun(){if(!G.run)return null;G.run.secretMarks=Math.max(0,Math.floor(G.run.secretMarks||0));G.run.secretRoomsFound=Math.max(0,Math.floor(G.run.secretRoomsFound||0));G.run.secretFloorGap=Math.max(0,Math.floor(G.run.secretFloorGap||0));G.run.secretShopsSeen=Math.max(0,Math.floor(G.run.secretShopsSeen||0));G.run.secretForge=G.run.secretForge&&typeof G.run.secretForge==='object'?G.run.secretForge:{};return G.run;}
